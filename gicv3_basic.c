...
uint32_t enableGIC(void)
{
  ...
  gic_dist->GICD_CTLR = GICD_CTLR_ARE_NS | GICD_CTLR_ARE_S | GICD_CTLR_ENABLE_G1A;

  // Wait for the RWP to be cleared - this means that the previous register writes went through
  while(gic_dist->GICD_CTLR & ( GICD_CTLR_RWP )) {
...
uint32_t setIntType(uint32_t ID, uint32_t rd, uint32_t type)
{
  ...
  if (ID < 31)
  {
    ...
    if(ID < 16){
      // SGIs configuration fixed
      return 1;
    }

    // See 12-646 in Arm IHI 0069H (ID020922) - NOTE: QEMU ignores this completely
    // The Generic Timers in ARM are level triggered, so we need to configure
    // GICR_ICFGR to match 
    uint32_t r_icgr0_val = gic_rdist[rd].sgis.GICR_ICFGR[1];
    if(type == GICV3_CONFIG_LEVEL){
      r_icgr0_val &= ~( (3u) << (2*ID) );
    } else {
      r_icgr0_val &= ~( (3u) << (2*ID) );
      r_icgr0_val |=  ( (2u) << (2*ID) );
    }
    gic_rdist[rd].sgis.GICR_ICFGR[1] = r_icgr0_val;    
  }
  else if (ID < 1020)
  {
    // SPI
...
uint32_t set_interrupt_sgi_ppi(uint32_t ID, uint32_t rd)
{
  ID = 1U << ID;                             // Move a '1' into the correct bit position
  gic_rdist[rd].sgis.GICR_ISPENDR[0] = ID;   // Note, there are three of these registers
                                             // but only one of them is used.

  volatile int val = gic_rdist[rd].sgis.GICR_ISPENDR[0];  
}
